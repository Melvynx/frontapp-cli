...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  frontapp-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  frontapp-cli auth show\n  frontapp-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: frontapp-cli auth set <token>");
      return;
    }
